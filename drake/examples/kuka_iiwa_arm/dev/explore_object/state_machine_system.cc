...
robot_plan_t MakeDefaultIiwaPlan() {
  robot_plan_t default_plan{};
...
struct ExploreObjectStateMachineSystem::InternalState {
  InternalState(const std::string& iiwa_model_path,
                const std::string& end_effector_name,
                const std::vector<Isometry3<double>>& grasp_locations)
      : world_state(iiwa_model_path, end_effector_name),
        state_machine(grasp_locations, false),
  ...
  ExploreObjectStateMachine state_machine;
  robot_plan_t last_iiwa_plan;
  lcmt_schunk_wsg_command last_wsg_command;
};

ExploreObjectStateMachineSystem::ExploreObjectStateMachineSystem(
  ...
  output_port_iiwa_plan_ =
      this->DeclareAbstractOutputPort(
              MakeDefaultIiwaPlan(),
              &ExploreObjectStateMachineSystem::CalcIiwaPlan)
          .get_index();

  output_port_wsg_command_ =
      this->DeclareAbstractOutputPort(
              MakeDefaultWsgCommand(),
              &ExploreObjectStateMachineSystem::CalcWsgCommand)
          .get_index();
...
ExploreObjectStateMachineSystem::AllocateAbstractState() const {
...
void ExploreObjectStateMachineSystem::SetDefaultState(
...
void ExploreObjectStateMachineSystem::CalcIiwaPlan(
    const systems::Context<double>& context,
    robot_plan_t* iiwa_plan) const {
...
void ExploreObjectStateMachineSystem::CalcWsgCommand(
    const systems::Context<double>& context,
...
void ExploreObjectStateMachineSystem::DoCalcUnrestrictedUpdate(
    const systems::Context<double>& context,
  ...
  // TODO(kmuhlrad): do I really need to specify the class here?
...
explore_object::ExploreObjectState ExploreObjectStateMachineSystem::state(
...
const pick_and_place::WorldState& ExploreObjectStateMachineSystem::world_state(
    const systems::Context<double>& context) const {
  const InternalState& internal_state =
      context.get_abstract_state<InternalState>(0);
